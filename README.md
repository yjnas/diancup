...
- 🌐 **导航页** - 内置炫酷的赛博朋克风格导航页，支持多种卡片类型和组件集成
...
### 仪表板 - 系统监控
### 导航页
![导航页](docs/screenshots/navigation.png)
*赛博朋克风格导航页，支持多种卡片类型*
...
### 昱君探针
![昱君探针](docs/screenshots/yjtz.png)
*多服务器一键切换管理*
...
内置一个现代化的个人导航页面，采用赛博朋克风格设计，支持多种卡片类型、组件集成和响应式布局。
...
1. 进入**导航页**页面（访问 `/nav` 路径）
2. 点击右上角设置按钮，输入管理密码登录
3. 登录后可进行以下操作：
   - 添加/编辑/删除链接卡片
   - 管理分类分组
   - 上传背景图
   - 修改站点设置
   - 配置组件卡片（Emby/Jellyfin/系统监控）
   - 添加容器监控卡片
...
<details>
<summary><b>Q: 如何使用导航页功能？</b></summary>

A: 导航页功能使用方法：
1. 访问 `http://your-server-ip:9527/nav` 进入导航页
2. 点击右上角设置按钮登录管理
3. 登录后可添加链接、创建分组、配置组件卡片
4. 支持导入浏览器书签，自动获取网站图标
5. 可设置背景图、主题色等个性化选项
</details>

<details>
<summary><b>Q: 导航页支持哪些组件？</b></summary>

A: 目前支持以下组件：
- **Emby**：显示最近观看的媒体内容
- **Jellyfin**：显示最近观看的媒体内容
- **系统监控**：显示 CPU、内存、磁盘使用率
- **容器卡片**：实时显示 Docker 容器状态和资源占用
</details>
